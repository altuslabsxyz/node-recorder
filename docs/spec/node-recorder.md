...
This match is what keeps each host to its own incidents: node B queries `instance="B"`, so node A's firing alert is never in B's result set. The cooldown state is keyed on `node + alertname` for the same reason.

Decision: since PromQL's `=` is an exact string match, a `NODE_ID` that does not equal the label value Prometheus carries returns an empty set forever, which is indistinguishable from a healthy node. At startup the daemon therefore queries `up{<ALERT_NODE_LABEL>="<NODE_ID>"}` once and logs an error when nothing matches. `up` is used rather than `ALERTS` because it exists for every scrape target at all times, whereas `ALERTS` is legitimately empty whenever nothing is firing. The check is advisory and never blocks startup: Prometheus is routinely unreachable at boot, and under `Restart=always` a hard failure would be a crash loop rather than a diagnosis. It assumes `ALERT_NODE_LABEL` names a target label, true for the `instance` default.
...
8. Query the CometBFT RPC `num_unconfirmed_txs` endpoint and save the raw mempool status response.
9. Extract the incident time window from `/var/log/haproxy.log`, reaching into the previous day's rotated file when the window crosses the daily rotation boundary, and cap the extracted output at `HAPROXY_LOG_MAX_BYTES` (see HAProxy Logging Policy below).
10. Write the manifest and record any collection errors.
11. Compress all files and upload them to S3.
12. Send the S3 location and collection result to the Slack channel.
13. Clean up local temporary files and enter cooldown.
...
| Mempool status | CometBFT RPC `num_unconfirmed_txs` | Check pending tx backlog at incident time |
...
# The node's CometBFT home directory. No default; required, validated at
# daemon startup. $DAEMON_HOME/config/config.toml's [rpc] table is the only
# source for the RPC (laddr) and pprof (pprof_laddr) addresses -- there is
# no separate PPROF_URL or RPC URL setting to keep in sync with it.
DAEMON_HOME="<cometbft home dir, e.g. /home/user/.cometbft>"
...
Decision: `DAEMON_HOME` replaces `PPROF_URL` as the sole way Node Recorder learns this node's pprof and RPC addresses. Both `laddr` (RPC, used for the mempool status endpoint) and `pprof_laddr` live in the same `config.toml` `[rpc]` table the node itself already maintains; reading them from there means an operator changing the node's ports only edits one file, instead of also updating a Node Recorder setting that could silently drift out of sync. A bind-all host (`0.0.0.0`) is normalized to `127.0.0.1`, since Node Recorder always polls its own local node.
...
| Mempool RPC unavailable | Failure recorded, remaining artifact collection continues |
